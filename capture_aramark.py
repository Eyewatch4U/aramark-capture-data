"""
Meltwater Auto-Capture — Monitor Aramark (NSGroup / Eyewatch Intelligence Unit)
==============================================================================
Captura el snapshot del shareable report de Meltwater (Aramark, 4 tabs) y lo
INGESTA directamente al Cloudflare Worker del Monitor Aramark.

Reporte Meltwater (4 tabs):
  - ARAMARK
  - Benchmark Chile
...
Flujo:
  ...
  5. POSTea el JSON completo a WORKER_INGEST_URL con el header x-sync-token.
     El Worker corre extractMeltwater() y escribe latest_aramark en KV.
...
Por qué /ingest y no /update-url (ni Make):
  ...
  la URL no cuesta nada extra, elimina el bloqueo y evita la carrera por
  expiración de la STS URL firmada.
...
Validación de señal:
  ...
  tabs -> rows -> cards -> fragments sumando hits/totals/counts sobre los 4 tabs.
  Si da 0, NO se ingesta, para no pisar 'latest_aramark' con un snapshot vacío.
  (El Worker aplica el mismo guard del lado servidor y responde 422 si el
  capture procesado queda vacío.)

El debug (responses, page.html, log) queda en debug_output/ y el workflow lo
sube como artifact privado del run. NO se comitea el snapshot crudo.
"""
...
def compute_signal(data: dict) -> int:
    ...
    Recorre TODOS los tabs (ARAMARK, Benchmark Chile, Benchmark Internacional,
    Industria Minera), así que basta con que UNO traiga datos para pasar el guard.
    """
...
async def capture_snapshot() -> dict:
    ...
    async with async_playwright() as p:
        ...
        print("→ Navegando a Meltwater (Aramark)...", flush=True)
    ...
    if validated is None:
        ...
        raise RuntimeError(
            ...
            f"Correr en self-hosted runner (IP residencial) o con PROXY_URL residencial."
        )
    return validated
